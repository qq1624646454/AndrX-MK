...
# Tiandf @ 2012-09-24  speed up mm and mmm build
# Enable clean build only when full build
ifneq ($(ONE_SHOT_MAKEFILE),)
$(info module(mm or mmm) build...)
else ifneq ($(filter kernel,$(MAKECMDGOALS)),)
$(info module(kernel) build...)
else
...
ifeq (true,$(strip $(enable_target_debugging)))
  ...
  # huangzhl enable adb root in user version
  ADDITIONAL_DEFAULT_PROPERTIES += ro.debuggable=1
...
# Tiandf @ 2012-09-24 speen up kernel build
ifneq ($(filter kernel,$(MAKECMDGOALS)),)
# Kernel only build.
$(info WARNING: Kernel only build.)
$(info WARNING: You must build system dependency module of kernel through mm or full build first.)
include $(TOPDIR)build/target/board/Android.mk
else # full build
#
...
subdir_makefiles := \
	$(shell build/tools/findleaves.py --prune=$(OUT_DIR) --prune=.repo --prune=.git --prune=.svn --prune=modem $(subdirs) Android.mk)

$(foreach mk, $(subdir_makefiles), $(info including $(mk) ...)$(eval include $(mk)))

endif # $(filter kernel,$(MAKECMDGOALS))
# Tiandf end
